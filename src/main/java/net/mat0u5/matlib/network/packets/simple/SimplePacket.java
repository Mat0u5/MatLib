...
public abstract class SimplePacket<U extends CustomPacketPayload, V> {
    ...
    protected SimplePacket(String name) {
        this.name = name;
        if (registeredPackets.containsKey(this.name)) {
            MatLib.LOGGER.error("Simple packet duplicate key: "+this.name);
        }
        registeredPackets.put(this.name, this);
    }
    ...
    private void sendPacketToServer(CustomPacketPayload packet) {
        if (packet == null) {
            MatLib.LOGGER.error("Packet was not initialized correctly.");
            return;
        }

        ClientAccessor clientAccessor = MatLib.getClientAccessor();
